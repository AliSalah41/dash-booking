        ...
        <div class="col-xl-9 mx-auto mt-5">
            <div class="card border-top border-0 border-4 border-primary">
                <div class="card-body p-5">
                    ...
                    <hr>
                    <form class="row g-3" action='{{ route('about.update', $about->id) }}' method="POST">
                        @csrf
                        @method('put')
                        <div class="col-12">
                            ...
                            <div class="input-group"> <span class="input-group-text bg-transparent">
                                ...
                                <select class="form-control single-select" name="countries_id" class="mb-3">
                                        ...
                                        @foreach ($countries as $country)
                                            <option {{ $country->id == $about->countries_id ? 'selected' : '' }} value="{{ $country->id }}" {{ $country->id == $about->countries_id? 'selected': '' }}>{{ $country->country }}</option>
                                        @endforeach
                                    </select>
                            </div>
                            @error('countries_id')
                        ...
                        <div class="col-12 d-flex">
                            <button type="submit" class="btn btn-primary px-5 ms-auto">{{ __('words.update') }}</button>
                        </div>
                    </form>
                </div>
            </div>
